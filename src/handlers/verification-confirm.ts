...
// Tapping the button within the window lifts the restriction and records the
// verification; a tap after the window expires removes the member.
...
composer.on("message:new_chat_members", async (ctx, next) => {
  ...
  for (const user of ctx.message.new_chat_members ?? []) {
    ...
    data.pending[user.id] = { timestamp: now(), expiry: now() + VERIFICATION_WINDOW_MS, status: "pending" };
    try {
    ...
    logAction(data, chatId, { actor: 0, target: user.id, action: "join", reason: "joined the group" });
  }
  ...
  const text =
    ...
    `You have ${Math.round(VERIFICATION_WINDOW_MS / 1000)}s to tap the button and verify you're human — ` +
...
composer.callbackQuery("verification:confirm", async (ctx) => {
  ...
  const t = now();
  if (t > pending.expiry) {
    // Verification pressed after the window — remove the member and explain.
    pending.status = "expired";
    setTrusted(data, userId, false);
    upsertMember(data, userId, ctx.callbackQuery.from.first_name ?? "", {});
    logAction(data, chatId, { actor: 0, target: userId, action: "kick", reason: "verification timed out" });
    await putChat(chatId, data);
    try {
    ...
    await ctx.reply(
      `Verification timed out for ${displayName(ctx.callbackQuery.from.first_name ?? "", userId)} — they've been removed. ` +
        `They can rejoin and try again.`,
    );
    return;
  }

  // Verified within the window — lift the restriction.
  ...
  logAction(data, chatId, { actor: 0, target: userId, action: "verify", reason: "passed human check" });
  await putChat(chatId, data);
  try {
